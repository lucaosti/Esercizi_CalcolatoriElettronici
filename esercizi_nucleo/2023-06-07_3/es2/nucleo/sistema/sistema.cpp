...
// ( SOLUZIONE 2023-06-07
	...
	vaddr ret = map(cow_root, beg, end, BIT_US, [](vaddr v){
		paddr f = alloca_frame();
		memset(reinterpret_cast<void*>(f), 0, DIM_PAGINA);
		return f;
	});
	...
	if(ret != end){
		unmap(cow_root, beg, ret, [](vaddr, paddr p, int){ rilascia_frame(p); });
		return false;
	}
...
// ( SOLUZIONE 2023-06-07
	...
	for(tab_iter it(esecuzione->cr3, base(v, 0), DIM_PAGINA); it; it.next()){
		tab_entry& e = it.get_e();
		
		if(!(e & BIT_P)) // Non è valido
			return false;

		if(e & BIT_RW) // Ha già la possibilità di scrittura
			continue;

		paddr new_frame,
			  old_frame = extr_IND_FISICO(e);
