...
extern "C" void c_shmem_attach(natl id)
{
	...
	if(!shmem_valid(id)){
		flog(LOG_WARN, "shmem non valida");
		c_abort_p();
		return;
	}

	des_shmem* sh = &array_desshmem[id];
	vaddr beg = esecuzione->avail_addr,
		  end = beg + sh->npag*DIM_PAGINA;

	if(end >= fin_utn_c || end < beg){
		flog(LOG_WARN, "memoria piena");
		return;
	}

	fun f;
	f.fn = sh->first_frame_number;
	vaddr v = map(esecuzione->cr3, beg, end, BIT_RW | BIT_US, f);
	if(end != v){
		unmap(esecuzione->cr3, beg, v, [](vaddr, paddr, int){});
		flog(LOG_WARN, "mappatura non andata a buon fine");
		return;
	}
