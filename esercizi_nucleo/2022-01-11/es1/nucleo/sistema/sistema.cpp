...
// ( ESAME 2022-01-11
// se non si aggiustano correttamente le traduzioni, può accadere che lo stesso
// page fault venga generato all'infinito. Cerchiamo di riconoscere questa situazione
// e di fermare il sistema quando si verifica.
vaddr last_pf_v;	// indirizzo virtuale dell'ultimo page fault
natq  last_pf_pid;	// processo che ha causato l'ultimo page fault
vaddr last_pf_rip;	// indirizzo dell'istruzione che ha causato l'ultimo page fault
natq  last_pf_error;	// dettagli dell'ultimo page fault

// contiamo i page fault generati (serve al programma utente di test)
natq num_page_faults;

// copy_on_write fa tutto il lavoro. Deve riconoscere e gestire il caso copy-on-write.
// Deve restituire true quando lo ha riconosciuto, e false altrimenti.
bool copy_on_write(natq errore, vaddr v);
//   ESAME 2022-01-11 )
...
extern "C" void gestore_eccezioni(int tipo, natq errore, vaddr rip)
{
	...
	case 17:// alignment check
		...
		if (errore) {
			flog(LOG_WARN, "  errore all'offset 0x%2x della %s", (errore & ~0x7),
				errore & SE_IDT ? "IDT":
	...
	case 14:
		...
		v = readCR2();
// ( ESAME 2022-01-11
		// fermiamo il sistema se riceviamo due volte lo stesso page fault
		if (esecuzione->id == last_pf_pid && last_pf_v == v &&
				last_pf_rip == rip && last_pf_error == errore) {
			panic("DOPPIO PAGE FAULT: CONTROLLARE copy_on_write");
		}
		last_pf_pid   = esecuzione->id;
		last_pf_v     = v;
		last_pf_rip   = rip;
		last_pf_error = errore;

		if (copy_on_write(errore, v)) {
			num_page_faults++;
			return;
		}
		// non era copy-on-write: proseguiamo con la normale gestione del page fault
//   ESAME 2022-01-11 )
	...
	if (rip < reinterpret_cast<vaddr>(&end)) {
		// se l'errore è stato causato dal modulo sistema stesso
		// si tratta di un bug e non conviene proseguire
...
extern "C" void end_program() __attribute__((noreturn));
extern "C" void halt();
void dummy(natq i)
{
	while (processi)
...
extern "C" void main(paddr mbi)
{
	...
	flog(LOG_INFO, "Nucleo di Calcolatori Elettronici, v6.6.6");
	init_gdt();
...
//   SOLUZIONE 2022-01-11 )
	...
	vaddr beg = ini_utn_p;
	vaddr end = beg + npag * DIM_PAGINA;
	vaddr end_ = map(esecuzione->cr3, beg, end, BIT_US|BIT_RW,
			[](vaddr) {
				paddr p = alloca_frame();
				if (p)
					vdf[p/DIM_PAGINA].nma = 1;
				return p;
			});
	if (end_ != end) {
		unmap(esecuzione->cr3, beg, end, [](vaddr, paddr p, int) { rilascia_frame(p); });
		esecuzione->contesto[I_RAX] = 0; // nullptr
		return;
	}
	esecuzione->contesto[I_RAX] = beg;
	esecuzione->masize = npag;
}

// ( SOLUZIONE 2022-01-11 #4
//   SOLUZIONE 2022-01-11 )
extern "C" void c_macopy(natq dst)
{
	esecuzione->contesto[I_RAX] = false;

	des_proc *d = des_p(dst);
...
//   SOLUZIONE 2022-01-11 )
	...
	natq dim = esecuzione->masize * DIM_PAGINA;

	// primo passo: collezioniamo gli indirizzi fisici della ma
	paddr ps[MAX_MA_PAGES];
	int n = 0;
	for (tab_iter it(esecuzione->cr3, ini_utn_p, dim); it; it.next()) {
		// l'iteratore si ferma su ogni tabella, ma a noi interessano
		// solo quelle di livello 1 (in questo caso le foglie)
		if (!it.is_leaf())
			continue;
		// it si è fermato su un'entrata di una tabella di livello 1
		tab_entry& e = it.get_e();    // riferimento all'entrata
		// vaddr v = it.get_v(); // indirizzo virtuale della pagina tradotta
		// 			 // da questa entrata
		// 			 // (in questo caso non ci serve)
		paddr p = extr_IND_FISICO(e); // indirizzo fisico del frame che
					      // corrisponde a v
		ps[n++] = p;
	}
	// secondo passo: proviamo a mappare i frame nel processo destinario
	// (in sola lettura)
	n = 0;
	vaddr end = map(d->cr3, ini_utn_p, ini_utn_p + dim, BIT_US,
			[&](vaddr) { return ps[n++]; });
	if (end != ini_utn_p + dim) {
		unmap(d->cr3, ini_utn_p, end, [](vaddr, paddr, int) {});
		return;
	}
	// terzo passo: se map() è andata a buon fine terminiamo l'operazione:
...
bool copy_on_write(natq errore, vaddr v)
{
	// se il page fault non è stato causato dal meccanismo del copy-on-write
	// su una ma restituiamo false, in modo che gestore_eccezioni() prosegua
	// con l'abort del processo
	if ( !(errore & PF_WRITE) || !(errore & PF_USER) ||
			!esecuzione->masize ||
			v < ini_utn_p || v >= ini_utn_p + esecuzione->masize * DIM_PAGINA)  {
		return false;
	}
