...
// ( SOLUZIONE 2023-06-07
	...
	vaddr beg = ini_utn_w,
		  end = ini_utn_w + DIM_USR_COW;

	vaddr v = map(cow_root, beg, end, BIT_US, [](vaddr v){
	...
	if(v != end){
		unmap(cow_root, beg, v, [](vaddr, paddr p, int){ rilascia_frame(p); });
		return false;
	}
...
{
// ( SOLUZIONE 2023-06-07
	if(esecuzione->livello != LIV_UTENTE || v < ini_utn_w || v >= fin_utn_w)
		return false;
