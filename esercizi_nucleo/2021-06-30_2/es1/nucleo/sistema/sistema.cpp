...
{
	natl indice;
	paddr array[(fin_ma_p-ini_ma_p)/DIM_PAGINA];
...
extern "C" void c_masend(natl pid)
{
	...
	// Controlli
	if(!src->maowner){
		flog(LOG_WARN, "Processo sorgente non possiede una ma");
		c_abort_p();
		return;
	}
	if(src == dst){
		flog(LOG_WARN, "Processo tenta di auto-inviarsi una ma");
		c_abort_p();
		return;
	}
	if(!dst){
		flog(LOG_WARN, "Processo destinatario inesistente");
		return;
	}
	if(dst->maowner){
		flog(LOG_WARN, "Processo destinatario già possiede una ma");
		return;
	}
	if(dst->livello != LIV_UTENTE){
		flog(LOG_WARN, "Processo sorgente tenta di inviare una ma ad un processo sistema");
		c_abort_p();
		return;
	}
	...
	fun f;
	f.indice = 0;
	unmap(src->cr3, ini_ma_p, fin_ma_p, f);

	f.indice = 0;
	vaddr ret = map(dst->cr3, ini_ma_p, fin_ma_p, BIT_RW | BIT_US, f);
	if(ret != fin_ma_p){
		unmap(dst->cr3, ini_ma_p, ret, [](vaddr, paddr, int){});
		ret = map(src->cr3, ini_ma_p, fin_ma_p, BIT_RW | BIT_US, f);
		if(ret != fin_ma_p){
			flog(LOG_WARN, "Grave errore, non siamo riusciti a ripristinare la situazione");
			c_abort_p();
			return;
		}
	}

	// Sistemo le informazioni dei processi
	dst->maowner = true;
