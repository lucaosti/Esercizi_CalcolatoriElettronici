...
// ( SOLUZIONE 2023-06-07
	...
	if(!cow_root)
		return false;
	vaddr ret = map(cow_root, ini_utn_w, ini_utn_w+DIM_USR_COW, BIT_US, [](vaddr){
		paddr p = alloca_frame();
		memset(reinterpret_cast<void*>(p), 0, DIM_PAGINA);
		return p;
	});
	if(ret != ini_utn_w+DIM_USR_COW){
		unmap(cow_root, ini_utn_w, ret, [](vaddr, paddr p, int){ rilascia_frame(p); });
		return false;
	}
	return true;
//   SOLUZIONE 2023-06-07 )
...
// ( SOLUZIONE 2023-06-07
	...
	for(tab_iter it(esecuzione->cr3, base(v, 0)); it; it.next()){
		tab_entry& e = it.get_e();
