...
// ( ESAME 2022-02-16
	...
	// significativi per i processi monitored
	des_proc *monitor;
	vaddr last_cr2;
	bool  write_fault;
	natq ma_npag;
	...
	// monitor: in attesa in ma_wait
	// minitored: aspetta ma_map dopo page fault
	bool waiting;
//   ESAME 2022-02-16 )
...
// ( ESAME 2022-02-16
bool check_ma(vaddr v, natq error);
//   ESAME 2022-02-16 )
...
// ( ESAME 2022-02-16
	if (p->monitor) {
		// il processo che sta terminando ha un monitor,
		// che deve essere informato sulla terminazione e,
		// se stava aspettando nella ma_wait, deve essere anche svegliato
		des_proc *m = p->monitor;
		m->monitored = nullptr;
		if (m->waiting) {
			m->waiting = false;
			m->contesto[I_RAX] = 0;
	...
	if (p->monitored) {
		// il processo stava monitorando un altro processo,
		// che ora dobbiamo sganciare e, se era in attesa di una map,
		// anche distruggere
		des_proc *m = p->monitored;
		m->monitor = nullptr;
		m->last_cr2 = 0;
		m->write_fault = 0;
		p->monitored = nullptr;
		if (m->waiting) {
			flog(LOG_WARN, "Processo %d: distrutto per terminazione del monitor", m->id);
			processi--;
			distruggi_processo(m);
		}
	}
	if (p->ma_npag) {
		unmap(p->cr3, ini_utn_p, ini_utn_p + p->ma_npag * DIM_PAGINA, [](vaddr, paddr, int) {});
		p->ma_npag = 0;
	}
//   ESAME 2022-02-16 )
...
extern "C" void c_ma_wait()
{
	...
	if (esecuzione->pending_event) {
		...
		esecuzione->contesto[I_RDX] = m ? m->write_fault : false;
		return;
	}
...
// restituisce true sse v appartiene alla ma di p
bool in_ma(vaddr v, des_proc *p)
{
	return v >= ini_utn_p && v < ini_utn_p + p->ma_npag * DIM_PAGINA;
}

bool check_ma(vaddr v, natq error) {
	if (!esecuzione->monitor || !in_ma(v, esecuzione)) {
		return false;
	}

	esecuzione->last_cr2 = v;
	esecuzione->write_fault = (error & PF_WRITE);
