...
// ( ESAME 2021-09-15
const vaddr ini_view_p = ini_utn_p;
const vaddr fin_view_p = ini_view_p + dim_region(0) * MSG_AREA_PAGES;

// indirizzo virtuale base della view descritta da *w
vaddr view_beg(const des_view* w)
{
	return w->base;
}

// indirizzo virtuale limite della view descritta da *w
vaddr view_end(const des_view* w)
{
	return view_beg(w) + w->ma->npag * DIM_PAGINA;
}
//   ESAME 2021-09-15 )
...
// restituisce il puntatore alla view del processo p con indirizzo
// virtuale base pari a v.
// nullptr se tale view non esiste.
des_view* find_view(des_proc *p, vaddr v)
{
	int i;
	for (i = 0; i < MAX_MSG_AREA_VIEW; i++)
		if (p->view[i].base == v && p->view[i].ma)
			break;
	return (i < MAX_MSG_AREA_VIEW ? &p->view[i] : nullptr);
}

// restituisce un puntatore alla prima view della msg-area ma nel
// processo p.
// nullptr se il processo p non ha view di ma
des_view* find_view(des_proc *p, des_ma *ma)
{
	int i;
	for (i = 0; i < MAX_MSG_AREA_VIEW; i++)
		if (p->view[i].ma == ma)
...
des_ma array_des_ma[MAX_MSG_AREA];
int find_free_ma()
{
	int i;
	for (i = 0; i < MAX_MSG_AREA; i++)
		if (!array_des_ma[i].npag)
			break;
	return (i < MAX_MSG_AREA ? i : -1);
}

// restituisce l'indice di una view non valida nel processo p. 
// -1 se non ve ne sono.
int find_free_view(des_proc *p)
{
	int i;
	for (i = 0; i < MAX_MSG_AREA_VIEW; i++)
		if (!p->view[i].ma)
			break;
	return (i < MAX_MSG_AREA_VIEW ? i : -1);
}
...
extern "C" void c_macreate(natq npag)
{
	...
	// la nuova msg-area sarà accessibile a partire da next_view
	vaddr beg = esecuzione->next_view;
	natq size = npag * DIM_PAGINA;
	vaddr end = beg + size;

	// se è fuori dall'intervallo [ini_view_p, fin_view_p) non 
	// possiamo crearla
	if (end < beg || end > fin_view_p)
		return;
	...
	// cerchiamo uno slot disponibile per la nuova view
	int j = find_free_view(esecuzione);
	if (j < 0)
		return;
	...
	array_des_ma[i].npag = npag;
	array_des_ma[i].views = 1;
	// registriamo la ma nel processo
	esecuzione->view[j].base = beg;
	esecuzione->view[j].ma = &array_des_ma[i];
	// avanziamo next_view in modo che la prossima msg-area
	// (creata o rivevuta) non si sovrapponga a questa
	esecuzione->next_view = end;
}
//   ESAME 2021-09-15 )
